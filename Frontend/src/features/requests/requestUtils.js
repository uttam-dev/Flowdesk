...
export function getRowActions(row, roles) {
  ...
  if (canAssign(roles)) {
    const assignEnabled = status === REQUEST_STATUS.Approved || status === REQUEST_STATUS.Open
    actions.push({ key: 'assign', label: 'Assign', variant: 'primary', disabled: !assignEnabled })
  }
