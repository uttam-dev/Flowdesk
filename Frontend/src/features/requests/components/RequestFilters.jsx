...
const selectClass =
  "min-h-[44px] w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm transition focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer";
...
export function RequestFilters({
  ...
  const showCreateBtn =
    canCreateRequest(roles) && (!isManager || activeTab === "my");
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            ...
            >
              ...
              {categories.map((c, idx) => (
                <option
                  ...
                  value={c.categoryId != null ? String(c.categoryId) : ""}
                >
                  {c.categoryName}
                </option>
              ))}
        ...
        {showCreateBtn ? (
          <Link
            to="/requests/new"
            className="inline-flex min-h-[44px] shrink-0 w-full whitespace-nowrap items-center justify-center rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700 sm:w-auto"
          >
            Create request
          </Link>
        ) : null}
