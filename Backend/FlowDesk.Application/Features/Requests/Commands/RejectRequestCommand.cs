...
namespace FlowDesk.Application.Features.Requests.Commands
{
    ...
    public class RejectRequestCommandHandler(IRequestRepository requestRepository, IRequestHistoryRepository requestHistoryRepository) : IRequestHandler<RejectRequestCommand>
    {
        public async Task Handle(RejectRequestCommand request, CancellationToken cancellationToken)
        {
            ...
            if (fetchedRequest.Category!.IsApprovalRequired)
            {
