        ...
        {rows.length === 0 ? (
          ...
          rows.map((row) => (
            <TableRow key={row.categoryId}>
              ...
              <Td className="max-w-[12rem] truncate font-medium">{row.categoryName}</Td>
              ...
              <Td className="text-xs text-gray-600">{fmtDate(row.createdOn)}</Td>
              <Td className="text-xs text-gray-600">{fmtDate(row.updatedOn)}</Td>
              <Td className="text-right">
                <div className="flex flex-wrap justify-end gap-2">
                  <Button type="button" variant="secondary" className="min-h-0 px-2 py-1.5 text-xs" onClick={() => onEdit(row)}>
                    Edit
                  </Button>
                  {row.isActive ? (
                    <Button
