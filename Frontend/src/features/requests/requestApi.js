...
function mapCommentRow(raw) {
  ...
  return {
    ...
    commentText:
      raw.commentText ?? raw.CommentText ?? raw.text ?? raw.Text ?? "",
    userId:
      raw.userId ?? raw.UserId ?? raw.createdById ?? raw.CreatedById ?? null,
    userName:
      raw.userName ??
      raw.UserName ??
      raw.fullName ??
    ...
    roleName:
      ...
      "",
    createdOn: raw.createdOn ?? raw.CreatedOn ?? null,
    isCurrentUser: raw.isCurrentUser ?? raw.IsCurrentUser ?? false,
  };
}
...
/**
 * @param {string|number} id
 * @param {{ remarksId?: number, commentText?: string }} body
 */
...
export async function rejectRequestApi(id, body) {
  const { data } = await apiClient.patch(`${BASE}/${id}/reject`, {
    remarksId: Number(body.remarksId),
